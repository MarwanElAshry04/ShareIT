...
namespace ShareIT.Seeds
{
    ...
    public static class DefaultData
    {
        public static async Task SeedAsync(ApplicationDbContext context)
        {
            ...
            {
                new() { ComplainType = "Harassment",          ComplainType_ar = "تحرش",        definition = "Verbal, physical, or psychological harassment.", priority = 1, status = 1, CreatedOn = now },
                new() { ComplainType = "Safety Violation",    ComplainType_ar = "مخالفة سلامة", definition = "Unsafe conditions or breaches of safety rules.", priority = 1, status = 1, CreatedOn = now },
                new() { ComplainType = "Fraud & Corruption",  ComplainType_ar = "احتيال وفساد", definition = "Financial misconduct, bribery, or corruption.", priority = 1, status = 1, CreatedOn = now },
                new() { ComplainType = "Discrimination",      ComplainType_ar = "تمييز",        definition = "Unfair treatment based on protected traits.",   priority = 2, status = 1, CreatedOn = now },
                new() { ComplainType = "Process Improvement", ComplainType_ar = "تحسين العمليات", definition = "Ideas to improve workflows and processes.",   priority = 3, status = 1, CreatedOn = now },
                new() { ComplainType = "Product Idea",        ComplainType_ar = "فكرة منتج",    definition = "New product or feature suggestions.",          priority = 3, status = 1, CreatedOn = now },
                new() { ComplainType = "General Feedback",    ComplainType_ar = "ملاحظات عامة", definition = "General comments and observations.",           priority = 4, status = 1, CreatedOn = now },
            };
            ...
            {
                MakeTicket("SHR-2026-0001", TicketKind.Complaint,  types[0], "Reported inappropriate behavior from a colleague during a team meeting.",
                ...
                MakeTicket("SHR-2026-0002", TicketKind.Complaint,  types[1], "Fire exit on the 3rd floor is blocked by stored equipment.",
                    companies[1].Name, departments[6].Name, "In-Process", "Valid",   now.AddDays(-12), pwd,
                    Rep(null, null, null, "Anonymous", null, null)),

                MakeTicket("SHR-2026-0003", TicketKind.Complaint,  types[2], "Suspected irregularities in a recent supplier procurement process.",
                ...
                MakeTicket("SHR-2026-0004", TicketKind.Complaint,  types[3], "Concerns about unequal access to training opportunities.",
                ...
                MakeTicket("SHR-2026-0005", TicketKind.Suggestion, types[4], "Introduce a shared digital request tracker to cut email back-and-forth.",
                    null, null, "Initiate",   null, now.AddDays(-5),  pwd,
                    Rep("Omar Hassan", "omar.hassan@elsewedy.com", "01009998887", "Disclosed", 2, "EE1120")),

                MakeTicket("SHR-2026-0006", TicketKind.Suggestion, types[5], "Add a mobile app for field engineers to log work orders offline.",
                    null, null, "In-Process", null, now.AddDays(-9),  pwd,
                    Rep("Youssef Fathy", "youssef.fathy@elsewedy.com", "01555666777", "Disclosed", 3, "EE1150")),

                MakeTicket("SHR-2026-0007", TicketKind.Suggestion, types[4], "Offer flexible start times to ease morning traffic congestion.",
                    null, null, "Closed",     null, now.AddMonths(-1), pwd,
                    Rep("Laila Mahmoud", "laila.mahmoud@elsewedy.com", "01778889990", "Disclosed", 0, "EE1188")),

                MakeTicket("SHR-2026-0008", TicketKind.Feedback,   types[6], "The new onboarding program was clear and very helpful. Thank you!",
                    null, null, "Closed",     null, now.AddDays(-2),  pwd,
                    Rep("Ahmed Zaki", "ahmed.zaki@elsewedy.com", "01004445556", "Disclosed", 2, "EE1201")),

                MakeTicket("SHR-2026-0009", TicketKind.Feedback,   types[6], "Cafeteria menu variety has improved a lot this quarter.",
                    null, null, "Initiate",   null, now.AddDays(-1),  pwd,
                    Rep(null, null, null, "Anonymous", null, null)),

                MakeTicket("SHR-2026-0010", TicketKind.Complaint,  types[1], "Slippery floor near the warehouse loading bay after rain.",
                ...
                MakeTicket("SHR-2026-0011", TicketKind.Feedback,   types[6], "IT support response time has been excellent lately.",
                    null, null, "Closed",     null, now.AddMonths(-3), pwd,
                    Rep("Nour Ibrahim", "nour.ibrahim@elsewedy.com", "01008887776", "Disclosed", 2, "EE1260")),

                MakeTicket("SHR-2026-0012", TicketKind.Complaint,  types[0], "Repeated dismissive comments from a supervisor toward the team.",
        ...
        private static Ticket MakeTicket(
            string refNo, TicketKind kind, TicketType type, string desc,
